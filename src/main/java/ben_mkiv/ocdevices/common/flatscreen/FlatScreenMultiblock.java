...
public class FlatScreenMultiblock {
    ...
    private TileEntityMultiblockDisplay origin;
    private final FlatScreenHelper helper = new FlatScreenHelper();
    private int width = 0, height = 0;

    private final HashSet<TileEntityMultiblockDisplay> screens = new HashSet<>();

    private AxisAlignedBB boundingBox = new AxisAlignedBB(0, 0, 0, 0, 0, 0);

    private long timeCreated = 0;

    public FlatScreenMultiblock(TileEntityMultiblockDisplay screen){
    ...
    private void mergeIntoMultiBlock(TileEntityMultiblockDisplay screen){
        screens.add(screen);
    ...
    private HashMap<FlatScreenMultiblock, EnumFacing.Axis> getAdjacentMultiBlocks(ArrayList<EnumFacing.Axis> axis){
        ...
        for(TileEntityMultiblockDisplay screen : screens()){
            for(EnumFacing facing : EnumFacing.values()){
                ...
                TileEntityMultiblockDisplay offsetScreen = MultiPartHelper.getScreenFromTile(screen.getWorld().getTileEntity(position));
    ...
    private void mergeMultiblocks(FlatScreenMultiblock source){
        for(TileEntityMultiblockDisplay screen : source.screens())
    ...
    private void setNewOrigin(TileEntityMultiblockDisplay newOrigin){
        if(newOrigin != null)
            origin = newOrigin;
    }

    // returns the axis the screen cant merge along
    private ArrayList<EnumFacing.Axis> getMergeAxis(TileEntityMultiblockDisplay screen){
    ...
    public HashSet<TileEntityMultiblockDisplay> screens(){
        return screens;
    }
